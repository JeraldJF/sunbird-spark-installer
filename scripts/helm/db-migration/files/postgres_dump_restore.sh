...
# Dumps entire database and restores into YugabyteDB YSQL
##########################################################
...
PGPASSWORD="$POSTGRES_PASSWORD" psql \
    ...
    -c "SELECT 1" > /dev/null 2>&1 && log_success "PostgreSQL connection OK" || { log_error "Cannot connect to PostgreSQL"; exit 1; }

# Test YugabyteDB connection
log_info "Testing YugabyteDB connection..."
PGPASSWORD="$YUGABYTE_PASSWORD" psql \
    -h "$YUGABYTE_HOST" -p "$YUGABYTE_PORT" \
    -U "$YUGABYTE_USER" -d "$YUGABYTE_DB" \
    -c "SELECT 1" > /dev/null 2>&1 && log_success "YugabyteDB connection OK" || { log_error "Cannot connect to YugabyteDB"; exit 1; }

# Count rows in PostgreSQL BEFORE migration
log_info "Row counts in PostgreSQL BEFORE migration:"
while IFS='|' read -r tschema tname; do
    ...
    log_info "  [BEFORE] $tschema.$tname: $count rows (PostgreSQL)"
...
# Count rows in YugabyteDB AFTER migration and compare
